...
public:
    void register_session_info(
        ...
        const noise_context_type::pre_shared_key_type &pre_shared_key) const;
...
void essu::protocol_type::prepare(packet_type &pckt, network::buffer_address_type addr,
    ...
    try {
        ...
        for (std::size_t i = 0; i < pckt->units.size(); ++i) {
            ...
            if (i >= 2) {
                unit.header.flag = unit_type::flag_type::drop;
            }
            ...
            // Adds random padding
            {
                ...
                else if (unit.header.type == unit_type::payload_type::hole_punch)
                    payload_data_size = 8;
                else
                    payload_data_size = 0;
