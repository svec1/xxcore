...
private:
    static constexpr bool hybrid_kyber1024 =
        (_ecdh == ecdh_type::x25519_hybrid_kyber1024
         || _ecdh == ecdh_type::x448_hybrid_kyber1024);
    static constexpr ecdh_type ecdh =
        hybrid_kyber1024
            ? ecdh_type(static_cast<std::uint16_t>(_ecdh) ^ NOISE_DH_KYBER1024)
            : _ecdh;

    static constexpr struct {
        std::uint16_t prefix_id = NOISE_PREFIX_PSK;
        std::uint16_t dh_id     = static_cast<std::uint16_t>(ecdh);
        std::uint16_t cipher_id = NOISE_CIPHER_CHACHAPOLY;
        std::uint16_t hash_id   = NOISE_HASH_BLAKE2b;
        std::uint16_t hybrid_id = hybrid_kyber1024 ? NOISE_DH_KYBER1024 : NOISE_DH_NONE;
    } nid_static;
...
public:
    ...
    struct cipher_state {
        friend class noise_context<_ecdh>;
    ...
    public:
        ...
        void hkdf(std::span<noheap::rbyte> buffer, std::span<noheap::rbyte> key,
...
// Noise buffer view
template<ecdh_type _ecdh>
constexpr NoiseBuffer *noise_context<_ecdh>::noise_buffer_view::operator->() noexcept {
    return &buffer;
}
template<ecdh_type _ecdh>
constexpr NoiseBuffer &noise_context<_ecdh>::noise_buffer_view::operator*() noexcept {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::noise_buffer_view::set(std::span<noheap::rbyte> buffer,
                                                  std::size_t              payload_size) {
    noise_buffer_set_inout(this->buffer, reinterpret_cast<noheap::ubyte *>(buffer.data()),
                           payload_size, buffer.size());
}
template<ecdh_type _ecdh>
const NoiseBuffer &noise_context<_ecdh>::noise_buffer_view::get() const {
    return this->buffer;
}

// Cipher state
template<ecdh_type _ecdh>
noise_context<_ecdh>::cipher_state::cipher_state() {
    std::size_t ret;
...
template<ecdh_type _ecdh>
noise_context<_ecdh>::cipher_state::cipher_state(cipher_state &&handle) {
    this->operator=(std::move(handle));
}
template<ecdh_type _ecdh>
noise_context<_ecdh>::cipher_state &
    noise_context<_ecdh>::cipher_state::operator=(cipher_state &&handle) {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::encrypt(std::span<noheap::rbyte> buffer_ad) {
    ...
    if ((ret = noise_cipherstate_encrypt_with_ad(
             ...
             buffer_ad.size(), &*input_buffer))
        != NOISE_ERROR_NONE)
        handle_error(ret, "Failed to encrypt.");
}
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::decrypt(std::span<noheap::rbyte> buffer_ad) {
    check_completed_handshake();
    std::size_t ret;
    if ((ret = noise_cipherstate_decrypt_with_ad(
             decrypt_state, reinterpret_cast<noheap::ubyte *>(buffer_ad.data()),
             buffer_ad.size(), &*output_buffer))
        != NOISE_ERROR_NONE)
        handle_error(ret, "Failed to decrypt.");
}
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::pad() {
    std::size_t ret;
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::set_nonce(std::uint64_t nonce) {
    check_completed_handshake();
    std::size_t ret;
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::set_states(NoiseCipherState *_encrypt_state,
                                                    NoiseCipherState *_decrypt_state) {
    dump_states();
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::check_completed_handshake() {
    if (!initialized_states)
        handle_error(0, "The states is not initialized.");
}
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::dump() {
    if (!initialized_states)
        return;
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::cipher_state::dump_states() {
    std::size_t ret;
...
template<ecdh_type _ecdh>
noise_context<_ecdh>::hash_state::hash_state(hash_state &&handle) {
    this->operator=(std::move(handle));
}
template<ecdh_type _ecdh>
...
template<ecdh_type _ecdh>
noise_context<_ecdh>::hash_state::buffer_hash_type
    noise_context<_ecdh>::hash_state::get_hash(std::span<noheap::rbyte> buffer) {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::hash_state::hkdf(std::span<noheap::rbyte> buffer,
                                            std::span<noheap::rbyte> key,
                                            std::span<noheap::rbyte> output1,
                                            std::span<noheap::rbyte> output2) {
    std::size_t ret;
    if ((ret = noise_hashstate_hkdf(
             hashstate, reinterpret_cast<noheap::ubyte *>(key.data()), key.size(),
             reinterpret_cast<noheap::ubyte *>(buffer.data()), buffer.size(),
...
// Noise context
template<ecdh_type _ecdh>
noise_context<_ecdh>::noise_context() {
    std::size_t ret;
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::init(noise_pattern pattern, noise_role role) {
...
template<ecdh_type _ecdh>
noise_context<_ecdh>::noise_buffer_view &noise_context<_ecdh>::get_handshake_buffer() {
    return handshake_buffer;
}
template<ecdh_type _ecdh>
noise_context<_ecdh>::noise_buffer_view &
    noise_context<_ecdh>::get_handshake_payload_buffer() {
    return handshake_payload_buffer;
}
template<ecdh_type _ecdh>
noise_context<_ecdh>::cipher_state noise_context<_ecdh>::get_cipher_state() {
    return std::move(cipher_st);
}

template<ecdh_type _ecdh>
noise_context<_ecdh>::name_id noise_context<_ecdh>::get_name_id() const {
    name_id buffer_tmp{};
    noise_protocol_id_to_name(buffer_tmp.data(), buffer_tmp.size(), &nid);
    return buffer_tmp;
}
template<ecdh_type _ecdh>
noise_action noise_context<_ecdh>::get_action() const {
    return noise_action(noise_handshakestate_get_action(handshakestate));
}
template<ecdh_type _ecdh>
noise_role noise_context<_ecdh>::get_role() const {
    return noise_role(noise_handshakestate_get_role(handshakestate));
}

template<ecdh_type _ecdh>
void noise_context<_ecdh>::set_handshake_message() {
    if (handshake_payload_buffer.get().max_size > handshake_payload_size)
        handle_error(0, "Invalid size of handshake payload.");

    std::size_t ret;
    if ((ret = noise_handshakestate_write_message(
             handshakestate, &*handshake_buffer,
             handshake_payload_buffer->data ? &*handshake_payload_buffer : NULL))
        != NOISE_ERROR_NONE)
        handle_error(ret, "Failed to set handshake message.");
}
template<ecdh_type _ecdh>
void noise_context<_ecdh>::get_handshake_message() {
    if (handshake_payload_buffer.get().max_size > handshake_payload_size)
        handle_error(0, "Invalid size of handshake payload.");

    std::size_t ret;
    if ((ret = noise_handshakestate_read_message(
             handshakestate, &*handshake_buffer,
             handshake_payload_buffer->data ? &*handshake_payload_buffer : NULL))
        != NOISE_ERROR_NONE)
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::set_prologue(prologue_extention_type &&ext) {
...
template<ecdh_type _ecdh>
noise_context<_ecdh>::buffer_prologue_type noise_context<_ecdh>::get_prologue() {
    buffer_prologue_type buffer_tmp{};
    std::copy(buffer_tmp.begin(), buffer_tmp.end(), reinterpret_cast<char *>(&prologue));
    return buffer_tmp;
}
template<ecdh_type _ecdh>
noise_context<_ecdh>::dh_key_type noise_context<_ecdh>::get_remote_public_key() {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::set_pre_shared_key(const pre_shared_key_type &key) {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::set_remote_public_key(const dh_key_type &key) {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::set_local_keypair(const keypair_type &kp) {
...
template<ecdh_type _ecdh>
void noise_context<_ecdh>::handle_error(std::size_t      error,
